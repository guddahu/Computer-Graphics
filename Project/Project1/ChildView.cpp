...
{
	m_bluegill.LoadFile(L"textures/marble10.BMP");
	m_camera.Set(20., 10., 70, 0., 0., 0., 0., 1., 0.);
	...
	redbox->Box(1, -9, 1, 5, 5, 5, &m_bluegill);


	//CGrPtr<CGrMaterial> redpaint = new CGrMaterial;
	//redpaint->AmbientAndDiffuse(0.4f, 0.4f, 0.f,0.f);
	//redpaint->Specular(0.f, 0.f, 0.1f);
	//redpaint->SpecularOther(0.f, 0.f, 0.1f);
	//
	//scene->Child(redpaint);
	...
	//whitepaint->AmbientAndDiffuse(0.4f, 0.f, 0.f);
	////whitepaint->Specular(0.2f, 0.2f, 0.2f);
	////whitepaint->SpecularOther(0.f, 0.f, 0.f);
	////whitepaint->Specular(0.6f, 0.6f, 0.6f);
	//scene->Child(whitepaint);

	//CGrPtr<CGrComposite> whitebox = new CGrComposite;
	//whitepaint->Child(whitebox);
	//whitebox->Box(-9, -9, -9, 5, 5, 5);


	// A red box
	CGrPtr<CGrMaterial> redpaint1 = new CGrMaterial;
	redpaint1->AmbientAndDiffuse(0.4f, 0.0f, 0.0f);
	redpaint1->Specular(0.2f, 0.4f, 0.5f);
	redpaint1->SpecularOther(0.1f, 0.3f, 0.1f);

	scene->Child(redpaint1);

	CGrPtr<CGrComposite> slantbox = new CGrComposite;
	redpaint1->Child(slantbox);
	slantbox->SlantBox(-9, -9, -9, 5, 5, 5, 3);

	//// A green surface
	CGrPtr<CGrMaterial> whitepaint1 = new CGrMaterial;
	whitepaint1->AmbientAndDiffuse(0.4f, 0.4f, 0.4f);
	//whitepaint1->Specular(0.6f, 0.6f, 0.6f);
	whitepaint1->SpecularOther(0.2f, 0.2f, 0.2f);

	scene->Child(whitepaint1);

	CGrPtr<CGrComposite> whitebox1 = new CGrComposite;
	whitepaint1->Child(whitebox1);
	whitebox1->Box(-20, -20, -20, 30, 10, 30);
...
void CChildView::ConfigureRenderer(CGrRenderer* p_renderer)
{
	...
	GLfloat dim1[] = { 0.1f,  0.1f,  0.1f, 0.1f };

	p_renderer->AddLight(CGrPoint(0,0 , 2, 0),
		dim1, dim1, dim1);
}
